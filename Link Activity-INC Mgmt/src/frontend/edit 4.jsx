...
const Edit = () => {
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      const [project, incident, activityLabel, activityKey] =
        fieldVal.split("##");
  ...
  const onSubmit = async () => {
    try {
      if (!incidentType || !selectedProject) {
        await view.submit(null);
        return;
      }
      ...
      const valueToSave = `${selectedProject?.value || ""}##${
        incidentType?.value || ""
      }##${selectedActivity?.label || ""}##${selectedActivity?.value || ""}`;
