...
// Helper function to link issues in Jira
async function linkIssues({ inwardIssueKey, outwardIssueKey }) {
  ...
  console.log(`Linking issues: ${inwardIssueKey} -> ${outwardIssueKey}`);
  ...
  for (let [
    ...
    if (!activityGroupIssue) {
      ...
      activityGroupIssue = await createIssue({
        ...
        customfield_10061: Number(standard?.toFixed(1)),
  ...
  while (!success && retryCount < 10) {
    try {
      ...
      try {
          ...
          .requestJira(route`/rest/api/3/issue/${issueId}`, {
            ...
            body: JSON.stringify({
              fields: {
                ...
                [customField10066]: Number(DFSValue?.toFixed(1)) || null,
        ...
        console.error("Error in updateKPI Event Listener:", error);
      }
...
export async function updateIssuesDateField(jql, dateFieldId) {
  ...
  console.log(`/rest/api/3/search?jql=${jqlEncoded}&maxResults=10`);
  const searchResponse = await retryJiraApiCall(() =>
    api
      .asApp()
      .requestJira(route`/rest/api/3/search?jql=${jqlEncoded}&maxResults=25`, {
        method: "GET",
        headers: {
          Accept: "application/json",
        },
